...
#include "/star/data01/pwg/xiatong/git/eff_11GeV/StRoot/StRefMultCorr/StRefMultCorr.h"
#include "/star/data01/pwg/xiatong/git/eff_11GeV/StRoot/StRefMultCorr/CentralityMaker.h"
...
void makeMuDstQA(TString InputFileList, Int_t cen, Int_t nFiles, Int_t nEvents, TString OutputFile)
{
   ...
   while (i <= nEvents && istat != 2)
   {
      ...
      // Run20 Au+Au 11.5 GeV triggers
      if (!mMuEvent->triggerIdCollection().nominal().isTrigger(710000)
      && !mMuEvent->triggerIdCollection().nominal().isTrigger(710010)
      && !mMuEvent->triggerIdCollection().nominal().isTrigger(710020))
         continue;
